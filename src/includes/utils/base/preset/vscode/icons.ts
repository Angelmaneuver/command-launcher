...
export class VSCodePreset {
	...
	public static icons: { [key: string]: { name: string, label: string, description: string }} = { ...iconA2E.icons, ...iconF2J.icons, ...iconK2O.icons, ...iconP2T.icons, ...iconU2Z.icons };

	public static get getAllIcons(): Array<QuickPickItem> {
		// eslint-disable-next-line @typescript-eslint/no-unused-vars
		return Object.entries(VSCodePreset.icons).map(([key, value]) => ({ label: `${value.label} ${value.name}`, description: '' }));
	}
	...
	public static create(
		...
		return {
			...
			description: 'string' === typeof description ? description : baseIcon.description
		};
	}
}
